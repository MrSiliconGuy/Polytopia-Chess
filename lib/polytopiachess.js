...
const GameFuncs = {
            ...
            if (v.r > 0) {
                ...
                for (const p of EIGHTDIRECTIONS) {
                    ...
                    for (const d of [...EIGHTDIRECTIONS, [0, 0]]) {
                        ...
                        if (GameFuncs.unitAtTribe(game, dLoc) !== null &&
    ...
    unitCanMove: function (game, unit) {
        ...
        if (unit.movements.recover == true ||
        ...
        if (unit.movements.move == true || unit.movements.attack == true) {
            if (unitTraits.includes("escape") &&
                unit.movements.attack == true && unit.movements.escape == false) {
                return true;
            } else {
                return false;
            }
    ...
    unitCanRecover: function (game, unit) {
        ...
        const unitTraits = UNITS[unit.type][5];
        if (unit.movements.recover == true || unit.movements.move == true || unit.movements.attack == true ||
            (unitTraits.includes("heal") && unit.movements.heal == true)) {
            return false;
            ...
            if (attStat[5].includes("splash")) {
                for (const t of game.tribes) {
                    if (t.name != GameFuncs.unitAtTribe(game, attSpot).name) {
                        let removeSpots = [];
                        for (const u of t.units) {
                            if (GameFuncs.distance(game, defSpot, u.location) <= 1 &&
                                !(u.location[0] == defSpot[0] && u.location[1] == defSpot[1])) {

                                const uStat = UNITS[u.type];
                                const uMaxHP = u.isVet ? uStat[2] + 5 : uStat[2];
                                let tempRes = calculate_formula([attStat[0] / 2, attUnit.health, attMaxHP], [uStat[0], u.health, uMaxHP], false, false, false);
                                if (tempRes[1] <= 0) {
